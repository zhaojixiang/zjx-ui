...
export default (props: PropsType) => {
  const { type, width = 300, height = 200 } = props
  useEffect(() => {
    switch (type) {
      case 'rect':
        drawRect()
  ...
  // 矩形
  const drawRect = () => {
    let draw: any = document.getElementById('rect');
    if (draw.getContext) {
  ...
  // 圆弧
  const drawArc = () => {
    let draw: any = document.getElementById('arc');
    if (draw.getContext) {
      const ctx = draw.getContext("2d");
      ctx.beginPath();
  ...
  const draw = () => {
    let canvas: any = document.getElementById('normal');
  ...
  return <div>
    <canvas id={type} width={width} height={height} />
  </div>
};
